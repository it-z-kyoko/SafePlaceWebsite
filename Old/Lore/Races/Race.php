...
$event = DBConnection::getRacebyId($id);
$chars = getCharacterRelatedToRace($id);
$race = getEventsRelatedtoRace($id);

if (isset($_POST['submit'])) {
    $uploadDirectory = __DIR__ . '/../../images/Races/';
...
if (isset($_POST['edit'])) {
    header('Location: EditRace.php?id=' . $id);
}

if (isset($_POST['create'])) {
    header('Location: CreateRace.php');
}
...
if (isset($_POST['sortr'])) {
    header('Location: SortRaceEvent.php?id=' . $id);
}
...
<body>
    <div class="div-2">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            <input type="submit" value="Volk Bearbeiten" name="edit">
            <input type="submit" value="Volk Erstellen" name="create">
            <input type="submit" value="Ordne Charaktere zu" name="sortc">
            <input type="submit" value="Ordne Events zu" name="sortr">
        </form>
        <div class="page-content">
            ...
            <div class="additional">
                <section class="personality">
                    ...
                    <p><?php echo $event->getPersonality() ?></p>
                    <h2>Körperbeschreibung</h2>
                    <p><?php echo $event->getBodyDescription() ?></p>
                    <h2>Beziehungen zu anderen Völkern</h2>
                    <p><?php echo $event->getRelationships() ?></p>
                    <h2>Moralische Ausrichtung</h2>
                    <p><?php echo $event->getAlignment() ?></p>
                    <h2>Ursprung</h2>
                    <p><?php echo $event->getLandOrigin() ?></p>
                    <h2>Religion</h2>
                    <p><?php echo $event->getReligion() ?></p>
                    <h2>Sprache</h2>
                    <p><?php echo $event->getLanguage() ?></p>
                    <h2>Geläufige Namen</h2>
                    <p><?php echo $event->getNames() ?></p>
